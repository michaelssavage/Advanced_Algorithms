""" The 8 puzzle programs that we looked at could find a path from the start to the goal, but didn't print them out.

One way to do this, is for a child node to remember its parent position. This can be done in a few ways.
One way is for the get_children() method to add a parent node indicating who the parent is.
Obviously a node that is creating children is the parent and so, each child should be allocated that as the parent.

Modify the Puzzle8Node code so that each child will have a get_parent() method which will return the parent.
If a Puzzle8Node does not have a parent, then get_parent() should return None.
...
class Puzzle8Node(Node):
   ...
   def test(self):
      ...
      assert len(moves) == 4
      #print(moves)
      print(moves == [ Puzzle8Node(move) for move in ['123 45678', '1 3425678', '12345 678', '1234756 8'] ])
