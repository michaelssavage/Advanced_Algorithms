"""Continuing on from the previous problem ...

Knowing the parent of the child makes it straightforward to get the complete path.
Add a get_path() method to the Puzzle8Node2 which returns the path starting from the current node and continuing until there is no parent (i.e. you reach the start.)
Start from your solution to the previous problem.

Note that your method will start from the end of the path and include every node in the path up to and including the start.

Your class should also provide a functioning get_parent() method.

Your class will be tested using the following program. Note however that the get_path method below is incomplete.
...
class Puzzle8Node2 (Node):
   ...
   def get_moves(self):
      ...
      return [Puzzle8Node2(move, Puzzle8Node2(self.name)) for move in moves]

   def test(self):
      p = Puzzle8Node2('2831647 5') # Lugar fig 3.17
      moves = p.get_moves()

      assert len(moves) == 3
      assert moves == [Puzzle8Node2(move) for move in ['283164 75', '2831 4765', '28316475 ']]

      p = Puzzle8Node2('1234 5678')
      moves = p.get_moves()
      assert len(moves) == 4
      #print(moves)
      assert moves == [ Puzzle8Node2(move) for move in ['123 45678', '1 3425678', '12345 678', '1234756 8'] ]


def main():
   Puzzle8Node2('2831647 5').test()
